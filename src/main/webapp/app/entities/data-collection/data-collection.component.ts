...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/data-collection'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: DataCollection) {
        return item.id;
    }
    registerChangeInDataCollections() {
        this.eventSubscriber = this.eventManager.subscribe('dataCollectionListModification', (response) => this.loadAll());
    }
